					...
					<?php
						...
						if(mysqli_num_rows($res2)>0){
							while($row = mysqli_fetch_assoc($res2)) {
								$type = $row["type"];
								$timer = $row["timer"];
								$strtotime = strtotime($timer);
								$time = date("h:i",$strtotime);
								if($type =="user" ){
									$class = "messages-me";
									$img = "image/user_avatar.png";
								}
								else{
									$class = "messages-you";
									$img = "image/bot_avatar.png";
								}
...
<strong class="messages-title">'.$type.'</strong> 
								<small class="time-messages text-muted">
								<span class="fas fa-time">
								</span>
								 <span class="minutes">'.$time.'</span>
								 </small>
								  </div>
								  <p class="messages-p">'.$row["message"].'</p></div></li>' ;
							  }
						}
					?>
                     </ul>
